...
import { ShieldCheck, ShieldAlert, Key, Loader2, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function LicensePage() {
  ...
  const handleActivate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        setMessage({ type: 'success', text: "¡NeuroScribe activado con éxito! Gracias por tu apoyo." });
  ...
  const trialDays = profile ? 30 - Math.ceil(Math.abs(new Date().getTime() - new Date(profile.trial_start_date).getTime()) / (1000 * 60 * 60 * 24)) : 0;
  const isExpired = !profile?.is_activated && trialDays <= 0;

  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">Licencia y Activación</h1>
      ...
      <div className={cn(
        ...
        <div className={cn(
          "p-3 rounded-xl",
          profile?.is_activated ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" :
          isExpired ? "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400" :
          "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-400"
        )}>
          {profile?.is_activated ? <ShieldCheck size={24} /> : isExpired ? <ShieldAlert size={24} /> : <Key size={24} />}
        </div>
        <div>
          <h2 className="text-lg font-bold text-zinc-900 dark:text-white">
            {profile?.is_activated ? "Versión Activada" : isExpired ? "Periodo de Prueba Expirado" : "Periodo de Prueba Activo"}
          </h2>
      ...
      {!profile?.is_activated && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm">
          <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-6">Activar NeuroScribe</h3>
          ...
          <div className="mt-8 pt-8 border-t border-zinc-100 dark:border-zinc-800">
            <h4 className="font-bold text-zinc-900 dark:text-white mb-4">¿No tienes una licencia?</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <a href="https://neuroscribe.app/buy" target="_blank" className="p-4 rounded-xl border border-zinc-200 hover:border-indigo-500 transition-all group">
                <span className="block font-bold text-zinc-900 dark:text-white group-hover:text-indigo-600 transition-colors">Pago Único (Lifetime)</span>
                <span className="text-sm text-zinc-500">$99 - Olvídate de suscripciones.</span>
              </a>
              <a href="https://neuroscribe.app/buy" target="_blank" className="p-4 rounded-xl border border-zinc-200 hover:border-indigo-500 transition-all group">
                <span className="block font-bold text-zinc-900 dark:text-white group-hover:text-indigo-600 transition-colors">Micro-Suscripción Anual</span>
                <span className="text-sm text-zinc-500">$10/año - Acceso completo.</span>
              </a>
            </div>
          </div>
        </div>
      ...
      <div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl p-6 text-center">
        <p className="text-xs text-zinc-400 dark:text-zinc-500">
          Esta licencia está vinculada a tu hardware para prevenir el uso compartido no autorizado.
        </p>
      </div>
    </div>
  );
