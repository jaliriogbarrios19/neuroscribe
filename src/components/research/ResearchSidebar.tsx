...
const ResearchSidebar = ({ isOpen, onClose, onInsertResult }: ResearchSidebarProps) => {
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex items-center justify-between px-1">
            <div className="flex items-center gap-2">
              <ShieldCheck size={14} className={cn(highPrecision ? "text-emerald-500" : "text-zinc-400")} />
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-tight">Alta Precisión (MeSH)</span>
            </div>
            <button 
              onClick={() => setHighPrecision(!highPrecision)}
              className={cn(
                "relative inline-flex h-5 w-9 items-center rounded-full transition-colors",
                highPrecision ? "bg-emerald-500" : "bg-zinc-200 dark:bg-zinc-700"
              )}
            >
              <span className={cn(
        ...
        <div className="flex-1 overflow-y-auto p-4 pt-0">
          {loading ? (
            <div className="flex h-40 flex-col items-center justify-center gap-2 text-zinc-500">
              ...
              <p className="text-xs">Consultando el Triángulo de la Verdad...</p>
            </div>
          ) : results.length > 0 ? (
            <div className="space-y-4">
              {results.map((work, idx) => (
                <div key={idx} className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-3 transition-colors hover:border-indigo-200 dark:border-zinc-800 dark:bg-zinc-800/50">
                  <h3 className="mb-1 text-sm font-bold leading-tight text-zinc-900 dark:text-white">
                    {work.title}
                  </h3>
                  <div className="flex flex-wrap gap-2 text-[10px]">
                    <span className="font-medium text-zinc-500">{work.year}</span>
                    <span className="truncate text-indigo-600 dark:text-indigo-400">{work.journal}</span>
                  </div>
                  ...
                  {work.mesh_terms && work.mesh_terms.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {work.mesh_terms.slice(0, 3).map((term, i) => (
                        <span key={i} className="flex items-center gap-0.5 rounded bg-zinc-100 dark:bg-zinc-700 px-1 py-0.5 text-[8px] text-zinc-500 dark:text-zinc-400">
                          <Tag size={8} /> {term}
                        </span>
                      ))}
                    </div>
            ...
            <div className="mt-20 text-center text-zinc-400">
              <p className="text-sm italic">Haz una pregunta para buscar evidencia rigurosa en PubMed y OpenAlex.</p>
            </div>
          )}
        </div>
