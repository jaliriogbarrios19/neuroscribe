...
const IAStatus = () => {
  ...
  if (loading) return <div className="p-4 flex items-center gap-2 text-zinc-500 text-xs font-medium"><Loader2 size={14} className="animate-spin text-indigo-500" /> Analizando hardware...</div>;
  ...
  return (
    <div className="p-4 bg-zinc-50 dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 space-y-4 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase ${hwInfo && hwInfo.total_ram_gb >= 15 ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-amber-100 text-amber-700 border border-amber-200'}`}>
          {hwInfo?.recommended_model === 'llama-3-8b' ? 'Modo Pro' : 'Modo Lite'}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 bg-white dark:bg-zinc-800/50 p-2 rounded-md border border-zinc-100 dark:border-zinc-800">
        <div className="space-y-0.5">
          ...
          <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-200">{hwInfo?.total_ram_gb} GB</p>
        </div>
        <div className="space-y-0.5 border-l border-zinc-100 dark:border-zinc-800 pl-3">
          ...
          <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-200">{hwInfo?.cpu_cores} Cores</p>
        </div>
      </div>
      ...
      <div className="space-y-3 pt-1">
        ...
        <div className="flex items-center justify-between group">
          <div className="flex items-center gap-2">
            <div className={`p-1 rounded ${modelStatus?.whisper_ready ? 'bg-green-50 dark:bg-green-900/20' : 'bg-zinc-100 dark:bg-zinc-800'}`}>
              <Database size={12} className={modelStatus?.whisper_ready ? 'text-green-600' : 'text-zinc-400'} />
            </div>
            <span className="text-[11px] font-semibold text-zinc-600 dark:text-zinc-300">Whisper (Audio)</span>
          </div>
          {modelStatus?.whisper_ready ? (
            <div className="flex items-center gap-1.5 text-green-600 font-bold text-[10px]">
              <span>Listo</span>
              <CheckCircle2 size={14} fill="currentColor" className="text-white dark:text-zinc-900" />
            </div>
          ) : downloading === 'whisper' ? (
        ...
        <div className="flex items-center justify-between group">
          <div className="flex items-center gap-2">
            <div className={`p-1 rounded ${modelStatus?.llama_ready ? 'bg-green-50 dark:bg-green-900/20' : 'bg-zinc-100 dark:bg-zinc-800'}`}>
              <Database size={12} className={modelStatus?.llama_ready ? 'text-green-600' : 'text-zinc-400'} />
            </div>
            <span className="text-[11px] font-semibold text-zinc-600 dark:text-zinc-300">Llama-3 (Psicología)</span>
          </div>
        ...
        <div className="flex items-center justify-between group">
          <div className="flex items-center gap-2">
            <div className={`p-1 rounded ${modelStatus?.biomed_ready ? 'bg-green-50 dark:bg-green-900/20' : 'bg-zinc-100 dark:bg-zinc-800'}`}>
              <Database size={12} className={modelStatus?.biomed_ready ? 'text-green-600' : 'text-zinc-400'} />
            </div>
            <span className="text-[11px] font-semibold text-zinc-600 dark:text-zinc-300">BioMedLM (Medicina)</span>
          </div>
      ...
      {!modelStatus?.whisper_ready && !downloading && (
        <div className="flex gap-2.5 p-2.5 bg-amber-50 dark:bg-amber-900/10 rounded-md border border-amber-100 dark:border-amber-900/20 mt-2">
          ...
          <p className="text-[10px] text-amber-800 dark:text-amber-400 leading-normal font-medium">
            Se requiere descargar los modelos para habilitar el procesamiento offline médico.
          </p>
        </div>
      )}
    </div>
