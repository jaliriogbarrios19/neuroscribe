...
const Sidebar = () => {
  ...
  const [openFolderView, setOpenFolderView] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  return (
    <>
            ...
            <nav className="space-y-1">
              {filteredFolders.map(folder => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'group flex w-full items-center justify-between rounded-md px-2 py-1.5 text-sm font-medium transition-colors',
                      isActive
              ...
              {filteredFolders.length === 0 && (
                <p className="text-[10px] text-center text-zinc-500 mt-4">
                  {searchTerm
                    ? 'Sin resultados.'
                    : 'No hay carpetas. Crea la primera.'}
                </p>
              )}
            </nav>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-zinc-500 truncate dark:text-zinc-400">
                Saldo: {profile?.minutes_balance || 0} min /{' '}
                {profile?.cc_balance || 0} CC
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Folder Document Explorer */}
      {openFolderView && (
        <FolderView
          folderId={openFolderView.id}
          folderName={openFolderView.name}
